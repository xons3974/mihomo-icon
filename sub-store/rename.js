...
// ─── 新增：为每个映射 key 预编译词边界正则，避免 IN 匹配 sin/kingdom 等 ───
// 规则：纯字母且长度 ≤ 3 的 key 用词边界 \b；中文/Emoji/长词用 includes 即可
function buildKeyMatcher(key) {
  if (/^[A-Za-z]{1,3}$/.test(key)) {
    // 纯英文短码（如 IN、US、GB）→ 词边界匹配，大小写不敏感
    const re = new RegExp("\\b" + key + "\\b", "i");
    return (name) => re.test(name);
  }
  // 其余（中文、Emoji、长词）→ 原 includes 逻辑
...
function operator(pro) {
  ...
  // ─── 修改：将 AMK 由 [key, value] 对扩展为 [key, value, matchFn] 三元组 ───
  const AMK = Object.entries(Allmap).map(([k, v]) => [k, v, buildKeyMatcher(k)]);
  ...
  pro.forEach((e) => {
    ...
    const matchedRureKey = Object.keys(rurekey).find(k => rurekey[k].test(e.name));
    if (matchedRureKey) {
      e.name = e.name.replace(rurekey[matchedRureKey], matchedRureKey);
      if (BLKEY) {
        bktf = true;
        let BLKEY_REPLACE = "", re = false;
        BLKEYS.forEach((i) => {
          if (i.includes(">") && ens.includes(i.split(">")[0])) {
            if (rurekey[matchedRureKey].test(i.split(">")[0])) {
              e.name += " " + i.split(">")[0];
            }
            if (i.split(">")[1]) {
    ...
    // ─── 修改：使用三元组的 matchFn 替代原 includes ───
    const findKey = AMK.find(([k, , matchFn]) => matchFn(e.name));
